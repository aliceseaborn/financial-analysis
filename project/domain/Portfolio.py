...
class Portfolio(object):
    ...
    def daily_individual_rates_of_return(self, start_date=None, end_date=None):
        ...
        for stock in self.stocks:
            ...
            df = pd.DataFrame(
                data=returns.iloc[:, 1].to_numpy(), columns=[stock.ticker + " Returns"]
            )
            dataframes.append(df)

        dataframes.insert(0, pd.DataFrame(data=returns["Date"], columns=["Date"]))
    ...
    def daily_combined_rate_of_return(self, start_date=None, end_date=None):
        ...
        for i in range(len(indiv_ret.columns) - 1):
            weighted_indiv_ret[indiv_ret.columns[i + 1]] = (
                indiv_ret[indiv_ret.columns[i + 1]].astype(float) * self.weights[i][0]
            )

        # Calculate the portfolio's daily rate of return
        total_returns = pd.Series(
            data=weighted_indiv_ret.sum(axis=1), name=self.name + " Returns"
        )
    ...
    def daily_excess_rates_of_return(
        self, risk_free_rate, start_date=None, end_date=None
    ):
        ...
        excess_daily_returns[self.name + " Excess Daily Returns"] = daily_returns.iloc[
            :, 1
        ] - (risk_free_rate / 250)
    ...
    def average_annual_rate_of_return(self, start_date=None, end_date=None):
        ...
        ave_annual_return = (
            self.average_daily_rate_of_return(start_date, end_date) * 250
        )
    ...
    def individual_variances(self, start_date=None, end_date=None):
        ...
        data = dict()
        for stock in self.stocks:
            data[stock.ticker + " Variance"] = [
                stock.annual_variance(start_date, end_date)
            ]
    ...
    def variance(self, start_date=None, end_date=None):
        ...
        portfolio_variance = (
            np.matmul(np.matmul(self.weights.T, cov_matrix.to_numpy()), self.weights)[
                0, 0
            ]
            * 100
        )
    ...
    def markowitz_efficient_frontier(
        self, density, risk_free_return, figure_path, start_date=None, end_date=None
    ):
        ...
        for i in range(density):
            ...
            returns.append(
                Simulation.average_annual_rate_of_return(start_date, end_date)
            )
            risks.append(Simulation.annual_standard_deviation(start_date, end_date))
            sharpe.append(
                Simulation.sharpe_ratio(risk_free_return, start_date, end_date)
            )
            data.append(
                dict(
                    {
        ...
        plt.scatter(risks, returns, c=sharpe, cmap=cm.cmaps_listed.get("winter"))
        plt.title(self.name + " Efficient Frontier")
        plt.xlabel("Annualized Standard Deviation")
        plt.ylabel("Annualized Return")

        norm = plt.Normalize(np.min(sharpe), np.max(sharpe))
        mappable = cm.ScalarMappable(norm=norm, cmap=cm.cmaps_listed.get("winter"))
        mappable.set_array([])
        ...
        plt.scatter(portfolio_risk, portfolio_return, c="red")
