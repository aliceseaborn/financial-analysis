...
class Stock(object):
    ...
    def __init__(self, ticker, name, history):
        try:
            ...
            if isinstance(history, pd.DataFrame):
                ...
                raise TypeError(f"History must be a Pandas DataFrame. Passed {type(history)}.")
        except TypeError:
            raise TypeError(f"Types must be (str, str, pd.DataFrame). Improper types" \
    ...
    def annualization_factor(self, start_date=None, end_date=None):
        ...
        selection = self.history.query(f"Date >= '{start_date}' and Date " \
                                       f"<= '{end_date}'").reset_index(drop=True)
    ...
    def daily_rate_of_return(self, mode='simple', start_date=None, end_date=None):
        ...
        daily_ror = daily_ror.rename(columns={"Date": "Date", \
                                                          "Adj Close": self.ticker + " ROR"})
        
        return daily_ror

    
    def average_daily_rate_of_return(self, mode='simple', start_date=None, end_date=None):
    ...
    def annual_rates_of_return(self, years):
        ...
        for i in range(len(years)-1):
            selection = self.history.query(f"Date >= '{years[i]}' and Date <= '{years[i+1]}'")
            annual_ror = round(((selection.iloc[len(selection)-1,5] / selection.iloc[0,5]) - 1) * 100, 2)
